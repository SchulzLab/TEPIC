		...
		for loop in chrLoops:
			for otherloop in chrLoops:
				...
				if(loop[6] <= otherloop[6]):# same resolution or lower
					continue
				if(loop[1] <= otherloop[1] and otherloop[2] <= loop[2]):
					inclusions[chrKey].append((loop, otherloop))
				elif(loop[3] <= otherloop[1] and otherloop[2] <= loop[4]):
					inclusions[chrKey].append((loop, otherloop))
				elif(loop[1] <= otherloop[3] and otherloop[4] <= loop[2]):
					inclusions[chrKey].append((loop, otherloop))
				elif(loop[3] <= otherloop[3] and otherloop[4] <= loop[4]):
					inclusions[chrKey].append((loop, otherloop))
